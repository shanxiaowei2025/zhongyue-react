 ...
 */
export const batchUpdatePermissions = (permissions: { id: number, permission_value: boolean }[]) => {
  return request.post<ApiResponse<{ success: number, failed: number }>>('/permissions/batch-update', { permissions })
}
...
export const checkPermission = (role_name: string, permission_name: string) => {
  return request.get<ApiResponse<{ permission_value: boolean, permission: Permission | null }>>('/permissions/check', { role_name, permission_name })
} 