...
const receiverMap: Record<SealType, string> = {
  中岳: '定兴县中岳会计服务有限公司',
  雄安: '定兴县中岳会计服务有限公司河北雄安分公司',
  高碑店: '定兴县中岳会计服务有限公司高碑店分公司',
  脉信: '保定脉信会计服务有限公司',
  金盾: '定兴县金盾企业管理咨询有限公司',
...
const ExpenseReceipt: React.FC<ExpenseReceiptProps> = ({
  ...
  const renderFeeDetails = () => {
    ...
    if (receipt?.feeItems && receipt.feeItems.length > 0) {
      ...
      addFeeItem('客户资料整理费', receipt?.customerDataOrganizationFee)
      addFeeItem('变更费', receipt?.changeFee)
      addFeeItem('行政许可费', receipt?.administrativeLicenseFee)
      addFeeItem('其他业务收费（基础）', receipt?.otherBusinessFee)
      addFeeItem('其他业务收费', receipt?.otherBusinessOutsourcingFee)
      addFeeItem('其他业务收费(特殊)', receipt?.otherBusinessSpecialFee)
